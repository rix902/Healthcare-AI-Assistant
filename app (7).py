...
# ── API key: read from Streamlit secrets ONLY ────────────────────────────────
def get_api_key() -> str:
    """Read Groq API key from st.secrets (set in Streamlit Cloud or .streamlit/secrets.toml)."""
    try:
...
def extract_text_from_image(image_bytes: bytes) -> str:
    ...
    return "\n".join(load_ocr().readtext(arr, detail=0))
...
- If the answer isn't there: "This information is not available in the provided document."
...
def generate_pdf_report(summary: str, language: str) -> bytes:
    ...
    title_s = ParagraphStyle("T", parent=styles["Title"],  fontSize=22, textColor=teal,  alignment=TA_CENTER, fontName="Helvetica-Bold", spaceAfter=4)
    sub_s   = ParagraphStyle("S", parent=styles["Normal"], fontSize=10, textColor=colors.grey, alignment=TA_CENTER, spaceAfter=16)
    body_s  = ParagraphStyle("B", parent=styles["Normal"], fontSize=11, leading=16, textColor=navy, spaceAfter=8)
    hdr_s   = ParagraphStyle("H", parent=styles["Heading2"], fontSize=13, textColor=teal, fontName="Helvetica-Bold", spaceAfter=6)
    ...
    for line in summary.split("\n"):
        ...
        if not line:
            ...
            story.append(Paragraph(line, hdr_s))
        else:
            story.append(Paragraph(line.replace("&","&amp;").replace("<","&lt;"), body_s))
    story += [Spacer(1,20), HRFlowable(width="100%", thickness=0.5, color=colors.grey),
              Paragraph("AI-generated. Review by a healthcare professional required.", sub_s)]
    doc.build(story)
    return buf.getvalue()

# ── Session state defaults ────────────────────────────────────────────────────
for k, v in {"extracted_text": "", "summary": "", "chat_history": [],
              "language": "English", "camera_active": False, "source_label": ""}.items():
...
with st.sidebar:
    ...
    st.caption(f"Summaries will be generated in **{LANGUAGE_NAMES[lang]}**")
...
<div class="hero-header">
  ...
  <div class="hero-content">
    ...
    <p class="hero-subtitle">Transform complex hospital discharge notes & medical reports<br>into clear, patient-friendly summaries — instantly.</p>
...
with tab_input:
    ...
    with col1:
        ...
        input_method = st.radio("method", ["PDF Report", "Image File", "Camera Capture"],
        ...
        if input_method == "PDF Report":
            ...
            if uploaded_pdf:
                ...
                if text:
                    ...
                    st.warning("⚠️ No text found – PDF may be scanned. Try uploading as image.")
            ...
            if st.session_state["camera_active"]:
                camera_image = st.camera_input("Point camera at document", label_visibility="collapsed")
                if camera_image:
    ...
    with col2:
        ...
        manual_text = st.text_area("manual", height=230,
                                    placeholder="Paste discharge notes, prescriptions, or any medical text here…",
                                    label_visibility="collapsed")
    ...
    # Extracted text preview + generate
    if st.session_state["extracted_text"]:
        st.markdown("---")
        ...
        with gcol2:
            if st.button("🚀 Generate Patient-Friendly Summary", use_container_width=True, type="primary"):
                if not get_api_key():
                    st.error("⚠️ GROQ_API_KEY not set in secrets.toml or environment.")
                else:
...
with tab_summary:
    if not st.session_state["summary"]:
        ...
        with dc2:
            pdf_bytes = generate_pdf_report(st.session_state["summary"], st.session_state["language"])
            st.download_button("⬇️ Download Summary as PDF", data=pdf_bytes,
                               file_name=f"medclear_{st.session_state['language'].lower()}.pdf",
                               mime="application/pdf", use_container_width=True, type="primary")

        st.markdown("""
        <div class="ai-warning">
          ⚠️ <strong>AI-Generated Content:</strong> This summary is intended to assist — not replace — professional medical advice.
          A qualified healthcare professional must review it before patient use.
        </div>
        """, unsafe_allow_html=True)
...
with tab_chat:
    if not st.session_state["extracted_text"]:
        ...
        q_cols = st.columns(2)
        for i, q in enumerate(["What medicines should I take?",
            ...
            with q_cols[i % 2]:
                if st.button(q, key=f"qb_{i}", use_container_width=True):
                    ...
                    with st.spinner("🤔 Thinking…"):
                        reply = chat_with_document(st.session_state["chat_history"],
        ...
        for msg in st.session_state["chat_history"]:
            if msg["role"] == "user":
                ...
                st.markdown(f'<div class="chat-bot"><strong>🏥 MedClear:</strong><br>{msg["content"]}</div>', unsafe_allow_html=True)
